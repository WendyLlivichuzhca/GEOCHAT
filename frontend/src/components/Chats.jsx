...
const COUNTRIES = [
  ...
  { name: 'MÈxico', code: 'MX', dial: '+52', flag: 'MX' },
  { name: 'EspaÒa', code: 'ES', dial: '+34', flag: 'ES' },
...
function documentTheme(extension) {
  ...
  if (['xls', 'xlsx', 'csv'].includes(ext)) {
    return {
      icon: 'XLS',
      accent: 'bg-sky-50 text-emerald-600 border-sky-100',
      strip: 'bg-sky-500',
    };
  }
    ...
    >
      ...
      <div className="flex items-center justify-between border-t border-slate-100 px-3 py-2 text-[12px] font-bold text-slate-500">
        ...
        <span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 ${
          mine ? 'bg-sky-50 text-sky-700' : 'bg-slate-100 text-slate-700'
        }`}>
          <Download size={13} />
          Abrir
        </span>
...
const Avatar = React.memo(function Avatar({ contact, size = 'md', showFlag = true }) {
  ...
  return (
    <div className={`relative ${sizes[size]} shrink-0 select-none`}>
      {imageUrl && !imgError ? (
        <div className={`w-full h-full rounded-2xl bg-[#f0f9ff] border border-[#7dd3fc] flex items-center justify-center overflow-hidden relative group`}>
        ...
        <div className="flex justify-between items-center min-h-[20px]">
          <div className="flex items-center gap-1 min-w-0 text-slate-500 pr-2">
            ...
            <span className="text-[12px] truncate leading-5 text-slate-500">
              {chatPreview(chat) || 'Inicia una conversaciÛn...'}
            </span>
          </div>
        ...
        <div className={`rounded-[18px] px-4 py-3 shadow-sm relative transition-all text-left ${
          ...
          {mine && message.agente_nombre && (
            <p className="text-[9px] font-black text-sky-200 uppercase tracking-widest mb-1.5 px-0.5 select-none">
              ?? Enviado por: {message.agente_nombre} ({message.agente_nombre.toLowerCase().includes('wendy') ? 'DueÒa' : 'Agente'})
            </p>
          )}
          ...
          {message.quoted_message_id && (
            <div 
              className="mb-2 p-2 rounded bg-black/5 dark:bg-white/5 border-l-4 border-[#0369a1] text-xs text-left cursor-pointer opacity-90 hover:opacity-100 transition-opacity flex flex-col gap-0.5"
              onClick={(e) => {
                e.stopPropagation();
              ...
              <div className="absolute bottom-9 left-0 bg-white border border-slate-200 shadow-xl rounded-full px-2 py-1 flex items-center gap-1.5 animate-in slide-in-from-bottom-2 duration-150 z-30">
                {quickEmojis.map((emoji) => (
                  <button
                    ...
                    className={`text-[16px] hover:scale-125 transition-transform duration-100 ${message.reaccion === emoji ? 'bg-sky-50 rounded-full p-0.5' : ''}`}
                  >
                    {emoji}
                  </button>
                ))}
            ...
            {showMenu && (
              <div className={`absolute bottom-9 ${mine ? 'right-0' : 'left-0'} bg-white border border-slate-200 shadow-2xl rounded-xl py-1.5 min-w-[150px] animate-in zoom-in-95 duration-150 z-30`}>
                ...
                <button
                  ...
                  className="w-full text-left px-3 py-1.5 hover:bg-slate-50 text-xs font-semibold text-slate-700 flex items-center gap-2"
                >
                  <Pin size={13} className={!!message.fijado ? 'text-sky-600 rotate-45' : 'text-slate-500'} />
...
export default function Chats({ user, onLogout }) {
  ...
  // Estados para edici√≥n r√°pida de contacto en barra lateral
  ...
  // Estados para a√±adir tag y campo
  ...
  const [allAgents, setAllAgents] = useState([]); // Por ahora vac√≠o hasta tener endpoint
      ...
      try {
        ...
        if (changedJid && payload.data?.source !== 'message-reaction-update') {
          setChats((prevChats) => {
            ...
            if (chatIndex === -1) {
              // Chat no est√° en lista todavÌa ‚Üí recargar lista completa
              setTimeout(() => loadChats({ silent: true }), 150);
              return prevChats;
            }
            ...
            // Para chat-update, bridge.js env√≠a: { last_message, last_type, last_timestamp }
            ...
            // Incrementar no-le√≠dos SOLO para mensajes entrantes (es_mio === false)
        ...
        // Si el chat afectado es el que est· abierto ? recargar mensajes o actualizar reacciÛn
        const currentChat = selectedChatRef.current;
        if (currentChat?.jid && changedJid === currentChat.jid) {
          ...
          // No llamamos loadChats aquÌ para no sobreescribir el orden
  ...
  const visibleChats = useMemo(() => {
    ...
    // Filtro por Estado (Le√≠dos, Abiertos, Cerrados)
    ...
    // Filtro por B√∫squeda (ya manejado por el fetch en loadChats, pero por si acaso re-filtramos)
    if (debouncedSearch) {
  ...
  const startAudioRecording = async () => {
    ...
    try {
      ...
      setMessageError('No se pudo acceder al micrÛfono. Revisa los permisos del navegador.');
    }
  };
  ...
  const handleSyncChat = async (chat) => {
    ...
    try {
      ...
      // Si el bridge est√° apagado, Python retorna 500 con mensaje de conexi√≥n rechazada
      if (!resp.ok) {
        const errData = await resp.json().catch(() => ({}));
        const msg = errData?.error || errData?.message || `Error ${resp.status}`;

        if (msg.includes('10061') || msg.includes('Connection refused') || msg.includes('denegÛ')) {
          throw new Error('El Bridge de WhatsApp no est· corriendo. InÌcialo con: node bridge.js --user-id=X --device-id=Y');
        }
        throw new Error(msg);
      }
      ...
      // Refrescar datos tras sincronizaci√≥n exitosa
      loadChats({ silent: true });
      ...
      setMessageError(err?.message || 'Error al sincronizar. Verifica que el Bridge estÈ corriendo.');
  ...
  const handleAddTag = async (tagId) => {
    ...
    try {
      ...
      console.error("Error aÒadiendo tag:", err);
    }
  };
  ...
  const handleSaveAllFields = async () => {
    ...
    try {
      ...
      // 2. Guardar el nuevo campo si se est√° creando y se seleccion√≥ una definici√≥n
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (selectedFile) {
        // Enviar como FormData si hay un archivo (nuevo o de galer√≠a)
        ...
        if (selectedFile.file instanceof File) {
          ...
          // Si es de la galer√≠a, enviamos la URL
  ...
  const runDeleteMessage = async (message, target) => {
    ...
    if (target === 'everyone') {
      ...
      showToast('Mensaje eliminado para mÌ');
    }
  };
  ...
  const handleReportContact = async (contact) => {
    if (!window.confirm(`øEst·s seguro de que deseas reportar al contacto ${contact.nombre || contact.telefono}?`)) return;
    try {
      ...
      if (data.success) {
        ...
        showToast('Contacto reportado con Èxito');
      } else {
  ...
  const handleForwardMessageSubmit = async () => {
    ...
    try {
      ...
      showToast('Mensaje reenviado con Èxito');
    } catch (err) {
      console.error(err);
  ...
  const handleBulkDelete = async () => {
    if (selectedMessageIds.length === 0) return;
    if (!window.confirm(`øEst·s seguro de que deseas eliminar estos ${selectedMessageIds.length} mensajes seleccionados para todos?`)) return;
  ...
  const handleBulkForward = () => {
    ...
    if (!mergedText) {
      showToast('No hay texto para reenviar en la selecciÛn');
      return;
    }
  ...
  const handleOpenNewChat = () => {
    ...
    // Resetear form pero mantener el dispositivo seleccionado para la pr√≥xima
  ...
  return (
    <div className="flex h-screen bg-[#f5f5f6] font-sans overflow-hidden selection:bg-sky-200/50">
        ...
        <div className="flex-1 flex gap-0 min-h-0">
          ...
          <div className="relative shrink-0 flex" style={{ width: sidebarWidth }}>
            ...
            <div className="p-3 flex items-center gap-2 border-b border-gray-100 bg-white shrink-0 relative">
                  ...
                  <div className="absolute top-full left-0 mt-2 w-[300px] bg-white rounded-3xl shadow-[0_10px_40px_rgba(0,0,0,0.1)] border border-slate-100 z-[100] overflow-hidden animate-in fade-in zoom-in-95 duration-200 origin-top-left">
                    <div className="p-6 space-y-6">
                      ...
                      <div className="flex flex-col">
                        {[
                          { id: 'unread', label: 'Conversaciones no leÌdas', count: filterCounts.unread },
                ...
                {showSort && (
                  <div className="absolute top-full right-0 mt-2 w-48 bg-white rounded-2xl shadow-xl border border-slate-100 z-[100] p-2 animate-in fade-in zoom-in-95 duration-200 origin-top-right">
                    {[
                      ...
                      { id: 'unread', label: 'No leÌdos primero', icon: <Bell size={14} /> },
              ...
              <button 
                onClick={async () => {
                  if (window.confirm('øMarcar todas las conversaciones como leÌdas?')) {
                    try {
                ...
                title="Marcar todos como leÌdos"
              >
                <CheckCheck size={16} />
              </button>
            </div>
            ...
            <div className="px-4 py-2 text-xs font-semibold text-slate-500 border-b border-slate-100 bg-[#f7f8fd] shrink-0">
              {visibleChats.length} {visibleChats.length === 1 ? 'conversaciÛn' : 'conversaciones'}
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-0 space-y-0 overflow-hidden">
              ...
              {isLoadingChats && !visibleChats.length ? (
                ...
                <div className="h-full flex flex-col items-center justify-center p-8 text-center">
                  <p className="text-sm font-semibold text-slate-400">Ninguna conversaciÛn</p>
                </div>
              )}
            </div>
          </aside>
          ...
          <div
            ...
            title="Ajustar tamaÒo"
          />
        </div>

          {/* ‚îÄ‚îÄ Ventana de chat ‚îÄ‚îÄ */}
          <section className="flex-1 min-w-[320px] bg-white border-r border-slate-200 shadow-none flex flex-col overflow-hidden relative">
            {selectedChat ? (
              <>
                ...
                <div className="h-[60px] bg-white border-b border-slate-200 flex items-center justify-between px-5 shrink-0">
                    ...
                    <div className="min-w-0 flex-1">
                      {isEditingName ? (
                        <div className="flex items-center gap-2">
                          <input
                            type="text"
                            className="bg-white border border-sky-200 rounded-lg px-2 py-1 text-sm font-black w-full focus:outline-none focus:ring-2 focus:ring-sky-500"
                            value={editingNameValue}
                            onChange={(e) => setEditingNameValue(e.target.value)}
                            onKeyDown={(e) => e.key === 'Enter' && handleRename()}
                            autoFocus
                          />
                          <button onClick={handleRename} className="p-1 text-emerald-600 hover:bg-sky-50 rounded">
                            <Check size={16} />
                          </button>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    >
                      {selectedChat.estado_lead === 'cerrado' ? 'Abrir conversaciÛn' : 'Cerrar conversaciÛn'}
                    </button>
                  </div>
                </div>
                ...
                {(() => {
                  ...
                  return (
                    <div className="bg-sky-50 border-b border-sky-100 px-6 py-2 flex items-center justify-between text-xs font-semibold text-sky-800 shrink-0 select-none relative z-30">
                      <div 
                        onClick={() => {
                      ...
                      >
                        <Pin size={12} className="rotate-45 text-sky-600 shrink-0" />
                        <span className="font-bold shrink-0">Mensaje fijado:</span>
                        <span className="truncate text-sky-600/90">{pinnedMessage.texto || pinnedMessage.nombre_archivo || 'Archivo'}</span>
                      </div>
                      ...
                      <div className="relative">
                        <button
                          ...
                          className="w-7 h-7 hover:bg-sky-100 rounded-full flex items-center justify-center text-sky-600 transition-colors"
                        >
                          <ChevronDown size={16} />
                        </button>
                        ...
                        return (
                          <EmptyState 
                            ...
                            text={filterStarredOnly 
                              ? "No has destacado ning˙n mensaje en esta conversaciÛn todavÌa." 
                              : "Este contacto todavÌa no tiene historial guardado en GeoCHAT."
                            } 
                          />
                        );
                      }
                      ...
                      <div className="flex-1 min-w-0">
                        {selectedFile && (
                          <div className="mb-3 flex animate-in slide-in-from-left-4 duration-300">
                            ...
                            {selectedFile.type === 'audio' && (
                              <div className="ml-3 min-w-0 flex-1 rounded-xl border border-sky-100 bg-sky-50 px-3 py-2">
                  ...
                  <div className="px-4 py-2.5 flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2 text-[#9ca3af]">
                                      ...
                                      <div className="grid grid-cols-4 gap-6">
                                        {recentFiles.slice(0, 8).map((file, idx) => {
                                          const getFileIcon = (name) => {
                                            ...
                                            if (['xls', 'xlsx', 'csv'].includes(ext)) return <div className="bg-sky-50 p-3 rounded-xl"><div className="w-10 h-10 bg-[#107c41] rounded-lg flex items-center justify-center text-white font-bold text-xl">X</div></div>;
                                          ...
                                          return (
                                            <div key={idx} className="group cursor-pointer" onClick={() => {
                                              ...
                                              <div className="aspect-square bg-white rounded-2xl border border-slate-100 shadow-sm flex items-center justify-center overflow-hidden group-hover:border-[#0ea5e9]/50 group-hover:shadow-md transition-all duration-300">
                                                {file.type === 'image' ? (
                          ...
                          <button
                            ...
                            title={user?.rol === 'visor' ? 'GrabaciÛn no permitida' : (isRecordingAudio ? 'Detener grabaciÛn' : 'Grabar audio')}
                            disabled={user?.rol === 'visor'}
                          >
                            <Mic size={18} />
                          </button>
              ...
              <EmptyState title="Selecciona un chat" text="Seleccione una conversaciÛn para iniciar" showLogo={true} />
            )}
          </section>

          {/* ‚îÄ‚îÄ Panel de contacto ‚îÄ‚îÄ */}
          {selectedChat && (
              ...
              <>
                  ...
                  <div className="flex-1 min-w-0 pr-6 text-left space-y-1">
                        ...
                        <div className="flex items-center border-b border-[#0ea5e9] pb-1 w-full">
                          ...
                          >
                            <span 
                              ...
                              className="w-4 h-4 rounded-full border border-slate-300 flex items-center justify-center text-[10px] font-bold text-slate-400 cursor-pointer select-none hover:border-[#0ea5e9] hover:text-[#0ea5e9] transition-colors leading-none"
                              style={{ lineHeight: 1 }}
                            >
                              ?
                            </span>
                            {showNameRulesTooltip && (
                              <div style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, zIndex: 300 }} className="w-56 bg-[#0f172a] text-white rounded-xl shadow-xl p-4 animate-in fade-in slide-in-from-top-1 duration-200">
                                ...
                                <ul className="space-y-1 text-[11px] font-bold text-slate-300">
                                  <li>ï Obligatorio</li>
                                  <li>ï M·ximo 100 caracteres</li>
                                  <li>ï Solo letras y espacios</li>
                                </ul>
                    ...
                    {isEditingSidebarEmail ? (
                          ...
                          >
                            ...
                            {showEmailRulesTooltip && (
                              <div style={{ position: 'absolute', top: 'calc(100% + 8px)', right: 0, zIndex: 300 }} className="w-52 bg-[#0f172a] text-white rounded-xl shadow-xl p-3 animate-in fade-in slide-in-from-top-1 duration-200">
                                ...
                                <ul className="space-y-1 text-[11px] font-bold text-slate-300">
                                  <li>ï Formato de correo v·lido</li>
                                  <li>ï Ejemplo: correo@dominio.com</li>
                                </ul>
                                <div className="absolute bottom-full right-1.5 translate-y-1 w-2.5 h-2.5 bg-[#0f172a] rotate-45" />
                              </div>
                            )}
                      ...
                      <div className="flex items-center gap-1.5 group cursor-pointer" onClick={() => {
                        ...
                        <p className="text-xs text-slate-400 truncate">{selectedChat.correo || 'Sin correo electrÛnico'}</p>
                    ...
                    <button
                      ...
                      disabled={isSyncing}
                      className="mt-2.5 flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#0ea5e9]/20 bg-[#0ea5e9]/5 hover:bg-[#0ea5e9]/10 text-[#0ea5e9] hover:text-[#4b4cbf] text-[11px] font-bold transition-all disabled:opacity-50 w-fit"
                ...
                <div className="flex-1 overflow-y-auto">
                  <div className="p-5 space-y-0">
                    ...
                    {/* ‚îÄ‚îÄ ACCORDEON 2: CAMPOS CUSTOMIZADOS ‚îÄ‚îÄ */}
                    <div className="border-b border-slate-200 bg-white">
                      <button 
                        ...
                        <div className="pb-5 space-y-4 animate-in slide-in-from-top-2 duration-200">
                          ...
                          <div className="space-y-3">
                            ...
                            {assignedFields.length === 0 && !isCreatingField && (
                              <p className="text-xs text-sky-300 font-medium italic">Este contacto no tiene campos personalizados.</p>
                            )}
                    ...
                    {/* ‚îÄ‚îÄ ACCORDEON 3: NOTAS DEL CONTACTO ‚îÄ‚îÄ */}
                    <div className="border-b border-slate-200 bg-white">
                      <button 
      ...
      {/* ‚ïê‚ïê MODAL NUEVA CONVERSACI√ìN ‚ïê‚ïê */}
      <Modal 
        isOpen={showNewChatModal} 
        onClose={() => setShowNewChatModal(false)} 
        title="Iniciar nueva conversaciÛn"
      >
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              >
                <option value="" disabled>selecciona una opciÛn</option>
                {devices.map((dev) => (
          ...
          <div className="space-y-2">
            <label className="text-xs font-semibold text-slate-500 ml-1">
              N˙mero de telÈfono <span className="text-rose-500">*</span>
            </label>
            <div className="flex gap-2">
              <div className="flex-1 phone-input-container">
                <PhoneInput
                  ...
                  placeholder="Ingrese n˙mero de telÈfono"
                  enableSearch={true}
                  searchPlaceholder="Buscar paÌs..."
                  searchStyle={{
                    margin: '8px',
                    width: 'calc(100% - 16px)',
                    height: '38px',
      ...
      >
        <div className="space-y-6">
          <div className="rounded-[1.35rem] border border-[#bae6fd] bg-gradient-to-br from-[#f8fffd] to-[#f0f9ff] p-4 shadow-[0_18px_40px_-28px_rgba(99,102,241,0.45)]">
            <div className="flex items-start gap-3">
              <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-[#0ea5e9] to-[#38bdf8] text-white shadow-lg shadow-sky-100/80">
                <Link size={18} />
              </div>
              <div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative">
              <div className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-[#0ea5e9]">
                <Link size={16} />
              </div>
              <input
                type="url"
                ...
                autoFocus
                className="w-full rounded-2xl border border-[#bae6fd] bg-white py-4 pl-11 pr-4 text-sm font-semibold text-slate-700 outline-none transition-all focus:border-[#0ea5e9] focus:ring-4 focus:ring-sky-100"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 pt-1">
            ...
            <button
              ...
              className="rounded-xl bg-[#0ea5e9] hover:bg-[#4b4cbf] py-3 text-sm font-semibold text-white transition-all disabled:cursor-not-allowed disabled:opacity-40"
            >
              Insertar enlace
            </button>
          </div>
      ...
      {/* Modal Personalizado de ConfirmaciÛn de EliminaciÛn */}
      {messageToDelete && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[99999] animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl p-6 overflow-hidden flex flex-col gap-4 animate-in zoom-in-95 duration-200 text-center">
            ...
            <h3 className="text-sm font-bold text-slate-800">øDeseas eliminar este mensaje?</h3>
