...
const CrearCampana = ({ user, onLogout }) => {
  ...
  const handleSubmit = async () => {
    ...
    if (tipo === 'canal') {
      if (!creatorAdmin) {
        setError('Selecciona el número creador que administrara el canal.');
        return;
      }
    } else {
      if (admins.length < 2) {
              ...
              <section className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-5 py-4 shadow-sm">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-500">{typeArticlePlural} {typePlural} se crearon automáticamente cuando se vayan llenando</p>
                  </div>
                </div>
                        ...
                        <div className="mt-3 space-y-3">
                          <div className="flex items-start gap-3 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-emerald-700">
                            ...
                            <div>
                              ...
                              <p className="text-xs">Cada {tipo} usara una variación diferente del nombre, reduciendo el riesgo de que WhatsApp bloquee tu cuenta por crear {typePlural} con nombres idénticos.</p>
                            </div>
                          </div>
              ...
              <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                {tipo === 'canal' ? (
                  <>
                    ...
                    <p className="mb-4 text-sm text-slate-500">Selecciona el número conectado que creara y administrara los canales</p>
                    {creatorAdmin && (
