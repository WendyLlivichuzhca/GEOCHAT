...
const Tableros = ({ user, onLogout }) => {
    ...
    const handleDeleteBoard = async () => {
        ...
        } catch (err) { alert("Error al eliminará"); }
    };
    ...
    const handleDeleteStage = async () => {
        ...
        } catch (err) { alert("Error al eliminará etapa"); }
    };
        ...
        <div className="flex h-screen bg-transparent font-sans selection:bg-emerald-100/50 overflow-hidden">
            ...
            <main className="ml-[21rem] mr-4 mt-3 mb-3 flex h-[calc(100vh-24px)] flex-1 flex-col overflow-hidden rounded-2xl bg-white shadow-[0_8px_30px_rgba(15,23,42,0.06)] border border-slate-100/50">
                <div className="flex-1 overflow-y-auto px-8 py-7 flex flex-col min-w-0">
                    ...
                    <p className="text-sm text-slate-400 font-medium mb-5">Organiza tus contactos en columnas dinámicas vinculadas a tus etiquetas.</p>
                </div>
                            ...
                            <div key={t.id} className="relative flex items-center group">
                                ...
                                {tableroActivo === t.id && (
                                    <div className="relative">
                                        <button onClick={(e) => { e.stopPropagation(); setShowOptions(!showOptions); }} className="p-1 hover:bg-[#f0f9ff] rounded-md transition-colors text-[#9ca3af]">
                                ...
                                <div key={col.id} className="w-[300px] flex flex-col shrink-0 max-h-full">
                                    <div className="flex flex-col mb-4 px-1 shrink-0 gap-3">
                                        <div className="flex items-center justify-between">
                                            ...
                                            <div className="relative">
                                                <button onClick={() => setOpenStageOptions(openStageOptions === col.id ? null : col.id)} className="text-[#9ca3af] hover:text-[#0ea5e9] transition-colors p-1">
                                                    <MoreVertical size={15}/>
                                                </button>
                                                {openStageOptions === col.id && (
                                                    <div className="absolute top-0 left-full ml-2 w-32 bg-white border border-[#bae6fd] shadow-xl rounded-xl z-[70] overflow-hidden animate-in fade-in slide-in-from-left-2 duration-200">
                                                        <button onClick={() => { setEditStageData({ id: col.id, nombre: col.nombre, tag_id: col.tag_id }); setShowEditStageModal(true); setOpenStageOptions(null); }} className="w-full text-left px-3 py-2.5 text-[11px] font-bold text-[#374151] hover:bg-[#f0f9ff] flex items-center gap-2">
                                                            <FileText size={13}/> Editar
                                                        </button>
                                    ...
                                    <div className="flex-1 overflow-y-auto space-y-3 pr-1 custom-scrollbar min-h-[150px]">
                                        {col.items && col.items.length > 0 ? (
                                            ...
                                            <div className="py-12 border border-dashed border-slate-200 rounded-3xl flex flex-col items-center justify-center text-center bg-white/50 group hover:border-emerald-500/50 hover:bg-white transition-all">
                                                ...
                                                <p className="text-[9px] font-bold uppercase tracking-wider text-slate-400">Vacío</p>
                                            </div>
                                        )}
                                        <button className="w-full py-3.5 border border-dashed border-slate-200 rounded-xl flex items-center justify-center gap-2 text-slate-400 hover:text-emerald-600 hover:border-emerald-500/50 hover:bg-emerald-50/10 transition-all font-bold text-[10px] uppercase tracking-wider mt-2 group active:scale-[0.98]">
                                            <Plus size={14} className="group-hover:rotate-90 transition-transform duration-300" /> Mover aquíí
                                        </button>
                                    </div>
                                </div>
                            ))}
            ...
            <style>{`.custom-scrollbar::-webkit-scrollbar { width: 6px; height: 6px; }.custom-scrollbar::-webkit-scrollbar-track { background: transparent; }.custom-scrollbar::-webkit-scrollbar-thumb { background: #bae6fd; border-radius: 10px; }.custom-scrollbar::-webkit-scrollbar-thumb:hover { background: #0ea5e9; }`}</style>
        </div>
    );
};
