    ...
    try:
        ...
        if config_raw:
            try:
                ...
                if cal_provider and cal_provider != "ninguno" and agent.get("objetivo") == "agendar_citas":
                    ...
                    if cal_proactivas:
                        calendar_text += (
                            f"- SUGERENCIA PROACTIVA: Ofrece de forma proactiva {cal_opciones_sugerir} de horarios "
                            f"disponibles al cliente en vez de preguntarle qué hora prefiere. IMPORTANTE: reparte esas "
                            f"opciones en DÍAS DIFERENTES dentro de la semana consultada (por ejemplo, un horario el "
                            f"miércoles, otro el jueves, otro el viernes) en vez de ofrecer varias horas del mismo día — "
                            f"así el cliente tiene variedad real para elegir, salvo que solo haya un día con espacio "
                            f"disponible en toda la semana.\n"
                        )
        ...
        if calServicios_raw:
            try:
                ...
                logger.error(f"Error parseando calServicios: {sv_err}")
        ...
        for iteration in range(6):
            ...
            def _lookup_duracion_servicio(servicio_pedido):
                ...
                # (ej. mando "blanqueamiento dental" cuando el servicio se llama solo
                # "Blanqueamiento") — sin esta tolerancia, la busqueda exacta fallaba, la
                # cita nunca se creaba, y la IA terminaba inventando una confirmacion falsa
                # tras varios intentos fallidos. Se intenta primero un match exacto y,
                # si no hay, se acepta que uno sea substring del otro.
        ...
        # dejar al cliente real de WhatsApp sin ningun mensaje.
